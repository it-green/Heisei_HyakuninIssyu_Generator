...
export default class WriteArea extends Vue {
    ...
    private purpleSheet: HTMLImageElement | null = null;
    ...
    // 画像を読み込む
    private sheets: Img = {
        'purple-sheet': require('../assets/sheets/purple.png'),
    };

    // 画像を描画する。
    private drawPurpleSheet() {
        if (this.ctx === null || this.purpleSheet === null) {
            return;
        }
        this.ctx.drawImage(this.purpleSheet, 0, 0);
    }
    // 短歌をcanvasに描画する。
    ...
    private mounted() {
        ...
        this.purpleSheet = new Image();

        this.purpleSheet.onload = () => {
            if (this.ctx === null || this.purpleSheet === null) {
                return;
            }
            this.drawPurpleSheet();
        ...
        this.purpleSheet.src = this.sheets['purple-sheet'];
    }
    @Watch('firstSong')
    @Watch('secondSong')
    @Watch('thirdSong')
