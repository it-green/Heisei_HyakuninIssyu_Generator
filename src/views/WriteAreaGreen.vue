...
export default class WriteArea extends Vue {
    ...
    private greenSheet: HTMLImageElement | null = null;
    ...
    // 画像を読み込む
    private sheets: Img = {
        'green-sheet': require('../assets/sheets/green.png'),
    };

    // 画像を描画する。
    private drawGreenSheet() {
        if (this.ctx === null || this.greenSheet === null) {
            return;
        }
        this.ctx.drawImage(this.greenSheet, 0, 0);
    }
    // 短歌をcanvasに描画する。
    ...
    private mounted() {
        ...
        this.greenSheet = new Image();

        this.greenSheet.onload = () => {
            if (this.ctx === null || this.greenSheet === null) {
                return;
            }
            this.drawGreenSheet();
        ...
        this.greenSheet.src = this.sheets['green-sheet'];
    }
    @Watch('firstSong')
    @Watch('secondSong')
    @Watch('thirdSong')
