...
export default class WriteArea extends Vue {
    ...
    private orangeSheet: HTMLImageElement | null = null;
    ...
    // 画像を読み込む
    private sheets: Img = {
        'orange-sheet': require('../assets/sheets/orange.png'),
    };

    // 画像を描画する。
    private drawOrangeSheet() {
        if (this.ctx === null || this.orangeSheet === null) {
            return;
        }
        this.ctx.drawImage(this.orangeSheet, 0, 0);
    }
    // 短歌をcanvasに描画する。
    ...
    private mounted() {
        ...
        this.orangeSheet = new Image();

        this.orangeSheet.onload = () => {
            if (this.ctx === null || this.orangeSheet === null) {
                return;
            }
            this.drawOrangeSheet();
        ...
        this.orangeSheet.src = this.sheets['orange-sheet'];
    }
    @Watch('firstSong')
    @Watch('secondSong')
    @Watch('thirdSong')
